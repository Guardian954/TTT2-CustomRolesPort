...
if SERVER then
	...
	hook.Add("PlayerTakeDamage", "SwapperNoDamage", function(ply, inflictor, killer, amount, dmginfo)
		if TakeNoDamage(ply, killer, ROLE_SWAPPER) or DealNoDamage(ply, killer, ROLE_SWAPPER) then
	...
	hook.Add("EntityTakeDamage", "SwapperEntityNoDamage", function(ply, dmginfo)
		if EntityDamage(ply, dmginfo, ROLE_SWAPPER) or TakeEnvironmentalDamage(ply, dmginfo, ROLE_SWAPPER) then
			dmginfo:ScaleDamage(0)
			dmginfo:SetDamage(0)
			return
		end
	...
	hook.Add("DoPlayerDeath", "SwapperItemGrab", function(victim, attacker, dmginfo)
		if victim:GetSubRole() == ROLE_SWAPPER and IsValid(attacker) and attacker:IsPlayer() then
			victim.weapons = GetPlayerWeapons(victim)
			attacker.weapons = GetPlayerWeapons(attacker)	
		end
	end)
